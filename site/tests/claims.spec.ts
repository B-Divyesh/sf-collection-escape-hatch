...
test('@claim:secret-redaction removes every documented secret value', async ({ page }) => {
  ...
  const names = ['token', 'access-token', 'id_token', 'refresh_token', 'api-key', 'apikey', 'key', 'secret', 'client_secret', 'signature', 'sig', 'authorization', 'credential', 'password', 'session', 'jwt'];
  ...
  await page.goto('/?demo=1');
  for (const sentinel of [...sourceSecrets, ...targetSecrets]) await expect(page.locator('body')).not.toContainText(sentinel);
});
